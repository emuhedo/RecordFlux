package body TLS_Handshake.Extensions is
   ...
   begin
      pragma Assume (TLS_Handshake.Extension.Is_Contained (Buffer (Types.Index_Type (Offset) .. Buffer'Last)));
      return TLS_Handshake.Extension.Is_Valid (Buffer (Types.Index_Type (Offset) .. Buffer'Last));
   end Valid_Next;
   ...
   begin
      ...
      Last := (First + Types.Length_Type (TLS_Handshake.Extension.Message_Length (Buffer (First .. Buffer'Last))) + (-1));
      Offset := Offset_Type (Last + 1);
      pragma Assume (TLS_Handshake.Extension.Is_Contained (Buffer (First .. Last)));
   end Get_Next;

end TLS_Handshake.Extensions;
