with TLS_Handshake.Certificate_Entry;

package TLS_Handshake.Certificate_Entries
  with SPARK_Mode
is
   ...
   procedure Get_First (Buffer : Types.Bytes; Offset : out Offset_Type; First : out Types.Index_Type; Last : out Types.Index_Type)
     with
       ...
       Post => ((First >= Buffer'First and then Last <= Buffer'Last) and then TLS_Handshake.Certificate_Entry.Is_Contained (Buffer (First .. Last)));
